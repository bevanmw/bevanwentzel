...
sequenceDiagram
    ...
    participant Tooling as ORM / Scripts

    User->>Agent: "Gen migrations"
    Agent->>Agent: Detect ambiguity in the prompt
    Agent->>Codebase: Inspect package.json, README, AGENTS.md, scripts
    Codebase-->>Agent: Hints about stack and workflow
    Agent->>Codebase: Search for schema files, ORM config, prior migrations
    Codebase-->>Agent: Possible migration patterns
    Agent->>Tooling: Infer migration command or file format
    Tooling-->>Agent: ORM CLI, custom script, or manual migration path
    alt Agent guessed correctly
        Agent->>Codebase: Generate migration using discovered workflow
        Codebase-->>Agent: Migration created
        Agent-->>User: Returns completed migration
    else Agent guessed incorrectly
        Agent->>Codebase: Runs wrong command or misses a required step
        Codebase-->>Agent: Incomplete or incorrect migration
        Agent-->>User: Returns draft needing correction
        User->>Agent: Clarifies the exact migration workflow
    end
```
...
sequenceDiagram
    ...
    User->>Agent: "Generate migrations"
    Agent->>Skill: Load migration skill
    Skill-->>Agent: Use drizzle-kit generate and then run verify:migrations
    Agent->>Codebase: Inspect only the schema and changed files
    Codebase-->>Agent: Relevant model changes
    Agent->>Tooling: Run the exact migration workflow from the skill
    Tooling-->>Agent: Migration created and verified
    Agent-->>User: Returns completed migration with much less exploration
```
