...
```mermaid
flowchart LR
    subgraph DEV["Developer understanding"]
        CODE["Codebase"]
    ...
    subgraph LLM["LLM understanding"]
        FOUND["Explored code"]
        INFERRED["Inferred patterns"]
        MISSING["Missing context"]
    end

    CODE --> FOUND
    PRACTICES --> INFERRED
    PATTERNS -. partially discovered .-> INFERRED
    EXPECTATIONS -. often omitted .-> MISSING

    style DEV fill:#f5efe6,stroke:#3b2f2f,stroke-width:2px,color:#1f1717
    style LLM fill:#dbeafe,stroke:#1d4ed8,stroke-width:2px,color:#172554
    style MISSING fill:#fecaca,stroke:#b91c1c,stroke-width:2px,color:#7f1d1d
```
...
sequenceDiagram
    ...
    User->>AI: Generate migrations
    AI->>Repo: Inspect docs and scripts
    Repo-->>AI: Partial clues
    AI->>Repo: Search schema and old migrations
    Repo-->>AI: More clues
    AI->>CLI: Guess command
    alt Correct guess
        CLI-->>AI: Migration created
        AI-->>User: Return result
    else Agent guessed incorrectly
        CLI-->>AI: Wrong or incomplete output
        AI-->>User: Needs correction
        User->>AI: Clarify workflow
    end
```
...
sequenceDiagram
    ...
    User->>AI: Generate migrations
    AI->>Skill: Load migration skill
    Skill-->>AI: Exact command and checks
    AI->>Repo: Inspect changed schema
    Repo-->>AI: Relevant changes
    AI->>CLI: Run known workflow
    CLI-->>AI: Migration verified
    AI-->>User: Return result
```
...
People and agents are writing skills all the time. It can be useful to reuse and install existing skills. [https://skills.sh/](https://skills.sh/) is one such source of skills. These can be installed globally on your machine or in the codebase. Codebase specific skills are especially useful because they are automatically available to your coworkers.
...
Skills are really a response to two persistent problems in agentic coding: too much context and not enough of the right context. Instead of stuffing everything into a giant `AGENTS.md` file or repeating the same instructions in every prompt, skills let you package specific workflows, conventions, and project knowledge so they are loaded only when relevant.

That makes agents cheaper to use and more reliable on tasks that would otherwise require exploration, guesswork, and follow-up corrections. More importantly, it shifts the job of the developer from constantly re-explaining the work to designing better operational context for the agent.

The key takeaway for using skills is not to consider it as just another AI tool but, developing a framework around which to structure your logic, thinking and tool usage.
